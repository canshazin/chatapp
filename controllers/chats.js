...
const s3 = new AWS.S3({
  accessKeyId: process.env.IAM_USER_ID,
  secretAccessKey: process.env.IAM_USER_KEY,
});
...
exports.temp_save_img = async (req, res, next) => {
  try {
    const msg = req.params.name;
...
exports.upload_img = async (req, res, next) => {
  try {
    const file = req.file;
    const gid = req.query.gid;
    console.log(
    ...
    const s3Params = {
      ...
      Key: `${gid}/${req.user.email}/${new Date()}_${file.originalname}`, // Unique file name
      Body: file.buffer,
